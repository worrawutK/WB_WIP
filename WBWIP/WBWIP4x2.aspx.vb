...
Public Class WBWIP4x2
    ...
    Private Sub Dialog()
        If Request.QueryString("RackNo") Is Nothing Then
            ScriptManager.RegisterStartupScript(Me, Page.GetType, "Script", "showDialog('WBWIP4x2.aspx?RackNo=WB-R01,CheckWBWIP.aspx?');", True)
            Exit Sub
        Else
            lbGotoProcess.Text = Request.QueryString("RackNo")
        End If
    ...
    Private Sub GetCache()
        ...
        For Each user As Control In Panel1.Controls
            If TypeOf user Is UserControl = True Then
                Dim UserControl As ControlPosition4x2 = user
